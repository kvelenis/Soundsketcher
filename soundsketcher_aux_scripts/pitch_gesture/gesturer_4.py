...
def get_peaks_per_frame(y, sr, n_fft=4096, hop_length=512, peak_rel_height=0.03, max_peaks=30):
    ...
    for frame in frames:
        ...
        idx, _ = find_peaks(spec, height=thr)
        ...
        if len(p_mags) > max_peaks:
            ...
            p_mags  = p_mags[keep]
        order = np.argsort(p_freqs)  # sort by freq ascending (helps modal tracking)
...
# ---------- DOMINANT RIDGE (STFT) ----------
def ridge_from_stft(y, sr, n_fft=8192, hop_length=512, fmin=50, fmax=4000, smooth_sigma=2.0):
    """
    Extract dominant spectral ridge per frame + crest (peak/median) confidence.
    Uses scipy.signal.stft for robustness.
    Returns: ridge_f (Hz), crest (unitless), times (s)
    """
    win = get_window("hann", n_fft, fftbins=True)
    noverlap = n_fft - hop_length
    freqs, times, Z = stft(y, fs=sr, window=win, nperseg=n_fft, noverlap=noverlap,
    ...
    if S_band.size == 0:
        return np.full(S.shape[1], np.nan), np.zeros(S.shape[1]), times

    ridge_idx = np.argmax(S_band, axis=0)
    ridge_f = freqs_band[ridge_idx]
    eps = 1e-12
    peak_vals = S_band[ridge_idx, np.arange(S_band.shape[1])]
    med_vals = np.median(S_band, axis=0) + eps
    crest = peak_vals / med_vals

    ridge_f = gaussian_filter1d(ridge_f.astype(float), sigma=smooth_sigma)
    crest = gaussian_filter1d(crest.astype(float), sigma=smooth_sigma)
    return ridge_f, crest, times

# ---------- FUSION: harmonic pitch + ridge ----------
def fuse_pitch_harm_ridge(f_harm, r2, f_ridge, crest, r2_thresh=1, crest_p95=None, smooth_sigma=2.0):
    """
    Confidence fusion:
      - If harmonic fit confident (r2 >= r2_thresh) -> use f_harm.
      - Else blend toward ridge using normalized crest as weight.
    """
    print(r2)
    T = max(len(f_harm), len(f_ridge))
    # Align lengths if STFT used different hop counts
    f_h = np.full(T, np.nan); f_h[:len(f_harm)] = f_harm
    r2a = np.zeros(T);        r2a[:len(r2)]     = r2
    fr  = np.full(T, np.nan); fr[:len(f_ridge)] = f_ridge
    cr  = np.zeros(T);        cr[:len(crest)]   = crest

    f_out = np.full(T, np.nan, dtype=float)

    if crest_p95 is None:
        finite_crest = cr[np.isfinite(cr)]
        crest_p95 = np.percentile(finite_crest, 95) if finite_crest.size else 1.0
    crest_w = np.clip(cr / (crest_p95 + 1e-12), 0.0, 1.0)

    for i in range(T):
        fh = f_h[i] if np.isfinite(f_h[i]) else np.nan
        fr_i = fr[i] if np.isfinite(fr[i]) else np.nan

        if np.isfinite(fh) and r2a[i] >= r2_thresh:
            f_out[i] = fh
        elif np.isfinite(fr_i) and (not np.isfinite(fh) or r2a[i] < r2_thresh):
            if np.isfinite(fh):
                w = crest_w[i]
                f_out[i] = (1 - w) * fh + w * fr_i
            else:
                f_out[i] = fr_i
        else:
            f_out[i] = np.nan

    # smooth fused for gesture
    if np.any(np.isfinite(f_out)):
        idx = np.arange(T)
        valid = np.isfinite(f_out)
        filled = f_out.copy()
        if np.any(valid):
            filled[~valid] = np.interp(idx[~valid], idx[valid], filled[valid])
            filled = gaussian_filter1d(filled, sigma=smooth_sigma)
            filled[~valid] = np.nan
            f_out = filled
...
def plot_monophonic_with_modal(times, f_pitch, r2, modes, max_freq=2000,
    ...
    # Pitch line: opacity follows confidence (still driven by harmonic r2)
    for i in range(1, len(times)):
    ...
    ax.set_title("Monophonic Perceptual Pitch w/ Ridge Fusion & Modal View")
    ax.grid(True)
...
if __name__ == "__main__":
    audio_path = "indefinite_enviromental.wav"  # <- set this
    y, sr = librosa.load(audio_path, sr=None)

    # 1) peaks & times (for harmonic pitch + modal ridges)
    hop = 512
    peaks_per_frame, times = get_peaks_per_frame(
        y, sr,
        n_fft=4096, hop_length=hop,
        peak_rel_height=0.03, max_peaks=40
    )

    # 2) monophonic virtual pitch + confidence
    f_pitch_harm, r2 = hybrid_gesture_pitch_monophonic(peaks_per_frame, r2_thresh=0.85)
    f_pitch_harm = fill_small_gaps(f_pitch_harm, times, max_gap_s=0.08)
    f_pitch_harm = smooth_contour(f_pitch_harm, times, sigma_s=0.025)

    # 3) modal ridges (top 3)
    modes = extract_modal_ridges(peaks_per_frame, times, topN=3, freq_tolerance_hz=100, smooth_sigma_s=0.02)

    # 4) dominant ridge from STFT (same hop to align with 'times')
    ridge_f, crest, times_ridge = ridge_from_stft(
        y, sr,
        n_fft=8192, hop_length=hop,
        fmin=50, fmax=4000, smooth_sigma=2.0
    )
    # (If times_ridge length differs slightly, fusion function will pad/truncate safely.)

    # 5) fuse harmonic pitch with ridge fallback
    f_pitch_fused = fuse_pitch_harm_ridge(
        f_harm=f_pitch_harm,
        r2=r2,
        f_ridge=ridge_f,
        crest=crest,
    ...
    # 6) plot & save (use 'times' from peaks so it matches modal ridges)
    plot_monophonic_with_modal(
        times, f_pitch_fused, r2, modes,
        max_freq=1000,
        save_path="fused_monophonic_with_modal.png"
    )